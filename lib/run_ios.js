...
module.exports = function(logger, config, cli, projectDir, finished) {
	var iosBuildPath = path.join(projectDir, 'build/iphone'),
		cucumberParams = cli.__cucumberParams;

    logger.info('Looking for tiapp.xml');

	tiapp.find(projectDir, function(err, obj) {

		if (err) {
			logger.error(err);
			throw 'An error occured whilst search for the tiapp.xml file';
		}
		...
		if ( ! fs.existsSync(iosBuildPath)) {
			logger.exception("iPhone build folder at "+iosBuildPath+"does not exist. Aborting.");
		}

		exec('calabash-ios', ['setup'], { cwd: iosBuildPath }, function() {
			logger.log('Calabash has been added to your project.');
			exec('cp', ['-r', 'features', iosBuildPath], null, function() {
				logger.log('Features Directory created on iOS build target.');
				exec('xcodebuild', ['-project', XcodeProject, '-configuration','Debug', '-target', XcodeTargetScheme, '-sdk','iphonesimulator'], { cwd: iosBuildPath }, function() {
					logger.log('building the -cal scheme');
					process.env['APP_BUNDLE_PATH'] = iosBinaryBuildPath;

					exec('cucumber', ['-p', 'ios'].concat(cucumberParams), { cwd: 'build/iphone' }, function() {
						finished();
					});
				});
			});
